    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="visible-xs">
                <div class="carouselobile" data-ride="carousel">
                    <?php $char_limit = 100; ?>
                    <?php $first="active"; $order=0;?>
                    @foreach ($events as $event)
                        <div class="{{$first}}" data-order="{{$order}}">
                            <a href="{{ action('EventsController@show',$event->id) }}"> {{ HTML::image('uploads/medium/'.$event->name.'',$event->name, ['class'=>'img-responsive']) }} </a>
                        </div>
                    @endforeach
                </div>
            </div>
